...
!local variables 
 buf = -8				!variable for buffer
 n = -12						!variable to calculate sine and cosine
.global	main
	.align	4
main:	save	%sp, -104, %sp
		...
		nop						!delay slot
openok:	mov		%o0,%l0			!storing file descriptor
		set		fmt,%o0			!printing table header
		call	printf			!printing table header
		nop						!delay slot
read:	mov		%l0,%o0			!loading file descriptor
		add		%fp,buf,%o1		!setting buffer arg
		mov		8,%o2		!setting number of bites to read
