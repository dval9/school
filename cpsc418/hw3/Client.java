...
public class Client 
{
	...
	debug("Receiving prime p from the server.");
	BigInteger primep = BigInteger.ZERO;
	try{
		primep = new BigInteger(CryptoUtilities.receive(in));
	}catch(Exception e){}
	debug("Received p = "+primep.toString());
	debug("Receiving primitive root g from the server");
	BigInteger geng = BigInteger.ZERO;
	try{
		geng = new BigInteger(CryptoUtilities.receive(in));
	}catch(Exception e){}
	debug("Received g = "+geng.toString());
	debug("Computing random exponent a.");
	BigInteger randoma = BigInteger.ZERO;
	while(true){
		randoma = new BigInteger(bitlength, new Random(System.currentTimeMillis()));
		if((randoma.compareTo(BigInteger.ZERO) >= 0) && (randoma.compareTo(primep.subtract(new BigInteger("2"))) <= 0))
			break;
	}
	debug("Computed a = "+randoma.toString());
	debug("Computing and sending public key");
	BigInteger gpowa = geng.modPow(randoma, primep);
	try{
		CryptoUtilities.send(gpowa.toByteArray(),out);
	}catch(Exception e){}
	debug("Sent g^a = "+gpowa.toString());
	debug("Receiving server's public key");
	BigInteger gpowb = BigInteger.ZERO;
	try{
		gpowb = new BigInteger(CryptoUtilities.receive(in));
	}catch(Exception e){}
	debug("Received g^b = "+gpowb.toString());
	debug("Computing the shared key");
	BigInteger sharedkey = gpowb.modPow(randoma, primep);
	...
	System.out.println ("Usage: java Client hostname port#");
	System.out.println("     or java Client hostname port# debug");
