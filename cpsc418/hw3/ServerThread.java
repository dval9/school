...
public class ServerThread extends Thread
{
    ...
    public void getKey() {
		...
		do{
			counter++;
			q = new BigInteger(bitlength, 4, new Random(System.currentTimeMillis()));
			primep = q.multiply(new BigInteger("2"));
			primep = primep.add(BigInteger.ONE);
			if(primep.isProbablePrime(4))
				haveprime = true;
		}while(!haveprime);
		try{
			CryptoUtilities.send(primep.toByteArray(),out);
		}catch(Exception e){}
		debug("Sent p = "+primep.toString()+" {took "+counter+" iterations)");	
		debug("Computing primitive root g");
		int numiter = 0;
		BigInteger primes = new BigInteger("2");
		BigInteger power = primep.subtract(BigInteger.ONE);
		do{
			numiter++;
			BigInteger pow2 = power.divide(new BigInteger("2"));
			BigInteger powq = power.divide(q);
			BigInteger rem2 = primes.modPow(pow2, primep);
			BigInteger remq = primes.modPow(powq, primep);
			if(!rem2.equals(BigInteger.ONE) && !remq.equals(BigInteger.ONE))
				break;
			primes = primes.nextProbablePrime();
		}while(true);
		try{
			CryptoUtilities.send(primes.toByteArray(),out);
		}catch(Exception e){}
		debug("Sent g = "+primes.toString()+" (took "+numiter+" iterations)");
		debug("Computing random exponent b.");
		BigInteger randomb;
		while(true){
			randomb = new BigInteger(bitlength, new Random(System.currentTimeMillis()));
			if((randomb.compareTo(BigInteger.ZERO) >= 0) && (randomb.compareTo(primep.subtract(new BigInteger("2"))) <= 0))
				break;
		}
		debug("Computed b = "+randomb.toString());
		debug("Computing and sending public key");
		BigInteger gpowb = primes.modPow(randomb, primep);
		try{
			CryptoUtilities.send(gpowb.toByteArray(),out);
		}catch(Exception e){}
		debug("Sent g^b = "+gpowb.toString());
		debug("Receiving client's public key");
		BigInteger gpowa = BigInteger.ZERO;
		try{
			gpowa = new BigInteger(CryptoUtilities.receive(in));
		}catch(Exception e){}
		debug("Received g^a = "+gpowa.toString());
		debug("Computing the shared key");
		BigInteger sharedkey = gpowa.modPow(randomb, primep);
		debug("Computed shared integer rep of key = "+sharedkey.toString());
		debug("Extracting AES key");
