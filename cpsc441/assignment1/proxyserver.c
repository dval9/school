...
int main(int argc, char **argv)
{
  ...
  for(;;){
    ...
    if(fork() == 0){
      ...
      char send_data[10000]; // data to write to socket
      ...
      int is_image = 0; // tell if image or htlm
      int is_text = 0;
      ...
      read_len = recv(active_socket, &receive_data, sizeof(receive_data), 0); /* read request from client */
      sprintf(send_data, receive_data, read_len); // copy message from browser 
	...
	perror("connect");
	exit(EXIT_FAILURE);
      }// remove the annoying keep connection thing that firefox does
      if(debug)printf("%d:sending message to server\n", getpid());
      for(i = 0; i < read_len; i++){
	if(strncmp(receive_data + i, "Connection:", 11) == 0)
	  break;
      }// setup end code of message
      *(send_data + i) = '\r';
      *(send_data + i + 1) = '\n';
      *(send_data + i + 2) = '\r';
      *(send_data + i + 3) = '\n';
      send(web_socket, send_data, i + 3, 0);
	...
	*(clength + (b2-b1)) = 0; // dunno if i need null terminated string, but sure 
	...
	if(clength1 % 2 == 0 || handle_it == 0){ // message is even length, do this, or if multiple times through here just short circuit because clength might have changed
      ...
      printf("%d:message sent to client, closing web connection\n", getpid());
