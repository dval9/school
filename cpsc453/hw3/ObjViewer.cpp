...
GLfloat lightIa = .1f;
GLfloat lightIp = 1.f;
GLuint ADSLightShaderF, ADSLightShaderG, ADSLightShaderP, ADSLightShaderT;
GLint locAmbientF, locAmbientG, locAmbientP, locAmbientT;
GLint locDiffuseF, locDiffuseG, locDiffuseP, locDiffuseT;
GLint locSpecularF, locSpecularG, locSpecularP, locSpecularT;
GLint locLightF, locLightG, locLightP, locLightT;
GLint locMVPF, locMVPG, locMVPP, locMVPT;
GLint locMVF, locMVG, locMVP, locMVT;
GLint locNMF, locNMG, locNMP, locNMT;
GLint g_shaderSelect = 4;
GLint g_viewModeSelect = 1;
GLint vvh,vvw;
CTriangleMesh g_triMesh;
GLint locTextureT;
GLuint texture; // This is used for texture binding.
const char* texfile;
...
// context. 
void SetupRC(char* objfile, char* texturefile)
{
	...
	for( int i = 0; i < (int)g_triMesh.m_faces.size(); i++ ) { //iterate over all the rows in the
		...
		triangleTexCoords[0][0] = g_triMesh.m_textures[g_triMesh.m_tvertices[i].x].x;
		triangleTexCoords[0][1] = g_triMesh.m_textures[g_triMesh.m_tvertices[i].x].y;
		triangleTexCoords[0][2] = g_triMesh.m_textures[g_triMesh.m_tvertices[i].x].z;

		triangleTexCoords[1][0] = g_triMesh.m_textures[g_triMesh.m_tvertices[i].y].x;
		triangleTexCoords[1][1] = g_triMesh.m_textures[g_triMesh.m_tvertices[i].y].y;
		triangleTexCoords[1][2] = g_triMesh.m_textures[g_triMesh.m_tvertices[i].y].z;

		triangleTexCoords[2][0] = g_triMesh.m_textures[g_triMesh.m_tvertices[i].z].x;
		triangleTexCoords[2][1] = g_triMesh.m_textures[g_triMesh.m_tvertices[i].z].y;
		triangleTexCoords[2][2] = g_triMesh.m_textures[g_triMesh.m_tvertices[i].z].z;
	...
	ADSLightShaderF = g_shaderManager.LoadShaderPairWithAttributes("./shaders/ADSFlat.vp",
			"./shaders/ADSFlat.fp", 2, GLT_ATTRIBUTE_VERTEX, "vVertex", GLT_ATTRIBUTE_NORMAL,
			"vNormal");
	ADSLightShaderG = g_shaderManager.LoadShaderPairWithAttributes("./shaders/ADSGouraud.vp",
			"./shaders/ADSGouraud.fp", 2, GLT_ATTRIBUTE_VERTEX, "vVertex", GLT_ATTRIBUTE_NORMAL,
			"vNormal");
	ADSLightShaderP = g_shaderManager.LoadShaderPairWithAttributes("./shaders/ADSPhong.vp",
			"./shaders/ADSPhong.fp", 2, GLT_ATTRIBUTE_VERTEX, "vVertex", GLT_ATTRIBUTE_NORMAL,
			"vNormal");
	ADSLightShaderT = g_shaderManager.LoadShaderPairWithAttributes("./shaders/ADSTexture.vp",
			"./shaders/ADSTexture.fp", 3, GLT_ATTRIBUTE_VERTEX, "vVertex", GLT_ATTRIBUTE_NORMAL,
			"vNormal", GLT_ATTRIBUTE_TEXTURE0, "vTexture0");
	...
	locAmbientG	= glGetUniformLocation(ADSLightShaderG, "ambientColor");
	locDiffuseG	= glGetUniformLocation(ADSLightShaderG, "diffuseColor");
	locSpecularG = glGetUniformLocation(ADSLightShaderG, "specularColor");
	locLightG = glGetUniformLocation(ADSLightShaderG, "vLightPosition");
	locMVPG = glGetUniformLocation(ADSLightShaderG, "mvpMatrix");
	locMVG = glGetUniformLocation(ADSLightShaderG, "mvMatrix");
	locNMG = glGetUniformLocation(ADSLightShaderG, "normalMatrix");
	...
	locAmbientT	= glGetUniformLocation(ADSLightShaderT, "ambientColor");
	locDiffuseT	= glGetUniformLocation(ADSLightShaderT, "diffuseColor");
	locSpecularT = glGetUniformLocation(ADSLightShaderT, "specularColor");
	locLightT = glGetUniformLocation(ADSLightShaderT, "vLightPosition");
	locMVPT	= glGetUniformLocation(ADSLightShaderT, "mvpMatrix");
	locMVT = glGetUniformLocation(ADSLightShaderT, "mvMatrix");
	locNMT = glGetUniformLocation(ADSLightShaderT, "normalMatrix");
	locTextureT = glGetUniformLocation(ADSLightShaderT, "colorMap");

	glGenTextures(1, &texture);
	glBindTexture(GL_TEXTURE_2D, texture);
	if (!LoadTGATexture(texturefile, GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, GL_REPEAT))
...
void KeyboardFunc(unsigned char key, int x, int y)
{
	...
	case '4':
		...
		break;
	case '5':
		LoadTGATexture(texfile, GL_NEAREST, GL_NEAREST, GL_REPEAT);
		break;
	case '6':
		LoadTGATexture(texfile, GL_LINEAR, GL_LINEAR, GL_REPEAT);
		break;
	case '7':
		LoadTGATexture(texfile, GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, GL_REPEAT);
		break;
	case '9':
...
void RenderScene(void)
{
	...
	if(g_shaderSelect==4){
		glBindTexture(GL_TEXTURE_2D, texture);
		glUseProgram(ADSLightShaderT);
		glUniform4fv(locAmbientT, 1, vAmbientColor);
		glUniform4fv(locDiffuseT, 1, vDiffuseColor);
		glUniform4fv(locSpecularT, 1, vSpecularColor);
		glUniform3fv(locLightT, 1, vEyeLight);
		glUniformMatrix4fv(locMVPT, 1, GL_FALSE, g_transformPipeline.GetModelViewProjectionMatrix());
		glUniformMatrix4fv(locMVT, 1, GL_FALSE, g_transformPipeline.GetModelViewMatrix());
		glUniformMatrix3fv(locNMT, 1, GL_FALSE, g_transformPipeline.GetNormalMatrix());
		glUniform1i(locTextureT, 0);
	}
...
void ShutdownRC()
{
	glDeleteProgram(ADSLightShaderF);
	glDeleteProgram(ADSLightShaderG);
	glDeleteProgram(ADSLightShaderP);
	glDeleteProgram(ADSLightShaderT);
