...
int main(void)
{
  char *stack1, *stack2, *stack3, *stack4, *stacktop1, *stacktop2, *stacktop3, *stacktop4;
  start = time(NULL);
  signal(SIGUSR2, sig_handler);
/* need to give each child a stack space in memory */
  stack1 = malloc(1024*1024);
  stack2 = malloc(1024*1024);
  stack3 = malloc(1024*1024);
  stack4 = malloc(1024*1024);
 /* stack grows down */
  stacktop1 = stack1+(1024*1024);
  stacktop2 = stack2+(1024*1024);
  stacktop3 = stack3+(1024*1024);
  stacktop4 = stack4+(1024*1024);
  /* each thread searches fourth of the space, split on the first character of the message */
  pid_t pid1 = clone(fun1, stacktop1, SIGCHLD, NULL);
  pid_t pid2 = clone(fun2, stacktop2, SIGCHLD, NULL);
  pid_t pid3 = clone(fun3, stacktop3, SIGCHLD, NULL);
  pid_t pid4 = clone(fun4, stacktop4, SIGCHLD, NULL);
...
/* populate language of possible characters*/
  ...
  /* only search every four values as four threads */
  for(i = 62; i >= 0; i-=4){
    for(j = 62; j >= 0; j--){
      for(k = 62; k >= 0; k--){
	for(l = 62; l >= 0; l--){
